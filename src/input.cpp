...
void input_manager::init_keycode_mapping()
{
    ...
    keyname_to_keycode["MOUSE_X1"] = MOUSE_BUTTON_X1;
    keyname_to_keycode["MOUSE_X2"] = MOUSE_BUTTON_X2;
}
...
std::string input_manager::get_keyname( int ch, input_event_t inp_type, bool portable ) const
{
    ...
    if( inp_type == input_event_t::keyboard ) {
        ...
        } else if( ch == MOUSE_BUTTON_X1 ) {
            raw = translate_marker_context( "key name", "MOUSE_X1" );
        } else if( ch == MOUSE_BUTTON_X2 ) {
            raw = translate_marker_context( "key name", "MOUSE_X2" );
        }
