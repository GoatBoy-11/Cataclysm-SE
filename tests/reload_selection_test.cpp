...
TEST_CASE("reload_loaded_revolver_rejects_speedloader", "[reload][reload_selection]") {
    ...
    REQUIRE(!speedloader_owner->put_in(
...
TEST_CASE("reload_stable_ordering", "[reload][reload_selection]") {
    ...
    REQUIRE(!more_owner->put_in(item::spawn("38_special", calendar::start_of_cataclysm, 6)));
    auto& more = who.i_add(std::move(more_owner));

    auto tie_first_owner = item::spawn("38_speedloader", calendar::start_of_cataclysm, 0);
    REQUIRE(!tie_first_owner->put_in(item::spawn("38_special", calendar::start_of_cataclysm, 3)));
    ...
    REQUIRE(!tie_second_owner->put_in(item::spawn("38_special", calendar::start_of_cataclysm, 3)));
    ...
    REQUIRE(!less_owner->put_in(item::spawn("38_special", calendar::start_of_cataclysm, 1)));
    auto& less = who.i_add(std::move(less_owner));

    auto expensive_owner = item::spawn("38_speedloader", calendar::start_of_cataclysm, 0);
    REQUIRE(!expensive_owner->put_in(item::spawn("38_special", calendar::start_of_cataclysm, 6)));
    auto& expensive = *expensive_owner;
    get_map().add_item_or_charges(who.bub_pos() + tripoint_east, std::move(expensive_owner));
...
TEST_CASE("reload_discovery_detachable_target_order", "[reload][reload_selection]") {
    ...
    auto& mod = *mod_owner;
    REQUIRE(!gun.put_in(std::move(mod_owner)));
    REQUIRE(mod.can_reload_with(itype_id("aux_pressurized_tank")));
    REQUIRE(!mod.put_in(item::spawn("aux_pressurized_tank", calendar::start_of_cataclysm, 0)));
    REQUIRE(gun.can_reload_with(itype_id("stanag30")));
    REQUIRE(!gun.put_in(item::spawn("stanag30", calendar::start_of_cataclysm, 0)));
    ...
    REQUIRE(!fuel_owner->put_in(item::spawn("napalm", calendar::start_of_cataclysm, 100)));
...
TEST_CASE("reload_discovery_respects_magazine_parent_ammo", "[reload][reload_selection]") {
    ...
    REQUIRE(!gun.put_in(std::move(magazine_owner)));
