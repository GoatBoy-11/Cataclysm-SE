...
**HEAD:** `4720960fb3` on **`main`**. Nothing pushed anywhere.
Full suite: **1,082 cases, 1,078 passed**, the four `vision_*` failures environmental
(see CLAUDE.md).
...
1. **AIM's inventory pane does not show pocketed items**, and the obvious fix is worse
   than the bug. AIM addresses items by *flat-inventory index* — `advanced_inv.cpp:1243`
   does `g->u.i_at( sitem->idx )`. Appending pane entries for pocketed items would give
   them synthetic indices resolving to the **wrong item**, so moving or examining one
   would act on something else. A correct fix changes AIM's addressing to `item *`. Same
   root: "drop all" from AIM (`advanced_inv.cpp:847`) iterates `0..inv_size()` and so
   drops only flat-inventory items.
2. **NPC AI reasons from the flat inventory only.** `npc.cpp:1734` computes food and
   drink need that way, so an NPC whose rations are pocketed believes it is starving;
   `npc.cpp:2005` has the same blind spot for item value. NPCs receive routed items now.
   Behavioural, not a crash — a scope decision, not obviously a bug to fix.
3. **`i_add_or_drop` routing is fenced off.** Routing it corrupts the map's active-item
   cache — `map_test`'s `check_submap_active_item_consistency` catches it, order-dependent
   in a full run, clean in isolation. Bisected to that change specifically. Debug spawns
   route at their own call site instead (`wish.cpp`). **Now that routed items get
   `on_pickup()`, that may well have been the cause — worth retrying.**
4. **Import CDDA items, wallet first.** Agreed as the next job after the pocket system.
   ...
   dependencies.
5. **`longest_side` on more items**, if wanted. 825 values were ported from CDDA for ids
...
## The GitHub fork — DO NOT DO THIS UNLESS ASKED

Unchanged from the previous handoff. Oliver has chosen the fork route and has explicitly
said not to create it yet. `gh` is authenticated as **GoatBoy-11** with `repo` scope, so
the capability is there; the decision to go this route is not permission to execute it.

Untracked and not part of any commit: `gfx/ChibiUltica/` — which is a **complete, working
